report 50105 "BDMR Customer Invoice"
{
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(DocumentExt; Header."External Document No.")  //WDC
            {
            }
            column(CompanyCity; companyinfo.City)  //WDC
            {
            }
            column(Groupe; Groupe)  //WDC 1809
            {
            }
            column(TotalRemise; TotalRemise)  //WDC 1809
            {
            }
            column(NumeroLigne; NumeroDescription) //WDC 1809
            {
            }
            ...
            column(CompanyRegister; companyinfo."Trade Register")  //WDC
            {
            }
            column(companyinfo_Capital; companyinfo."Stock Capital")  //WDC
            {
            }
            column(Paiement; Paiement)  //WDC
            {
            }
            column(IDPrelevement; "Direct Debit Mandate ID")  //WDC
            {
            }
            column(BDMRInvoiceNo; BDMRInvoiceNo)     //WDC
            {
            }
            column(Posting_Date; "Posting Date")     //WDC
            {
            }
            column(TVA_Intra; Cust."VAT Registration No.")  //WDC
            {
            }
            ...
            dataitem(WorkDescriptionLines; "Integer")
            {
                ...
                begin
                    ...
                    if WorkDescriptionLine <> '' then NumeroDescription += 1; //WDC 1809
                    Groupe := (NumeroDescription DIV 18); //WDC 1809
                    Groupe += 1;
                end;
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                begin
                    ...
                    if ("Line No." <> 0) then NumeroDescription += 1; //WDC 1809
                    Groupe := (NumeroDescription DIV 18);            //WDC 1809
                    Groupe += 1; //WDC 1809
                    VATAmountLine.Init();
            ...
            {
                DataItemTableView = SORTING(Number) WHERE(Number = FILTER(1 .. 18));
                Column(NumeroAjouter; Number + NumeroDescription)//061169
                {

                }
                trigger OnAfterGetRecord()
                begin
                    Groupe := (NumeroDescription DIV 18); //061169 
                    Groupe += 1;
                end;

                trigger OnPreDataItem()
                begin
                    SETRANGE(Number, 1, 18 - (NumeroDescription MOD 18));
                end;
            }
